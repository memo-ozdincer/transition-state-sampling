...
"""Multi-mode escape for GAD with ECKART-PROJECTED MASS-WEIGHTED Hessian.

Production-ready runner for noisy/perturbed starting geometries. This is the
go-to script for TS search when input structures are imperfect.

Key features:
1. Eckart-projected, mass-weighted Hessian for ALL eigenvector computations
2. Removes translation/rotation null modes via Eckart projection
3. Displacement-based plateau detection (more robust than GAD norm)
4. Multi-mode escape via v2 perturbation when stuck at high-index saddles
...
- GAD norm ~ force_mean (0.2-3 eV/A) stays moderate even at plateaus
- disp_from_last dropping to ~1uA is the true indicator (dt shrinks/caps)
...
def run_multi_mode_escape(
    ...
    *,
    ...
    - This is more robust than GAD norm, which stays ~0.1-1 eV/A even at plateaus
...
def main(
    ...
    *,
    ...
    script_name_prefix: str = "noisy-multi-mode",
) -> None:
    parser = argparse.ArgumentParser(
        description="Noisy geometry runner: Multi-mode escape for GAD (escape high-index saddles via v2 perturbation)."
    )
    ...
    if args.wandb:
        ...
        init_wandb_run(
            ...
            tags=[args.start_from, args.method, "multi-mode", str(args.dt_control), "noisy"],
            run_dir=out_dir,
        )
...
if __name__ == "__main__":
    ...
    main(default_calculator="scine", enforce_calculator=True, script_name_prefix="noisy-scine-multi-mode-eckartmw")
