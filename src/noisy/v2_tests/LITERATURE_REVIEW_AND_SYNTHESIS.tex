...
findings from versions~1 through~13 of the minimization code.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Convergence}: $\norm{\vect{f}}_\infty < 10^{-4}$\,eV/\AA\
    ...
    \geq -\tau_\text{relax}$.
  \item \textbf{Scale}: 287--300 samples per configuration, 10\,000--50\,000
    maximum steps.
\end{itemize}
...
\fbox{\parbox{0.85\textwidth}{%
  ...
  With 50k steps + spectral probes + late escape (v13), convergence
  reaches \textbf{98\%/97\%/97\%/93\%} at 0.5/1.0/1.5/2.0\,\AA.
  The gap from 0.5 to 2.0\,\AA\ shrinks from 51\,pts (v9) to 5\,pts (v13).
...
v13 & 50k + v12b kicks combined & \textbf{98/97/97/93\%} (full v12b, 50k); orthogonality confirmed & \textbf{Combined breakthrough} \\
\bottomrule
\end{longtable}
...
\textbf{Next steps} (all addressed by v13, Section~\ref{sec:v13}):
\begin{itemize}[nosep]
  \item \textbf{50k + v12b kicks}: \emph{Answered---93\% at $n=2.0$
    (Table~\ref{tab:v13}).  Orthogonality confirmed
    (Table~\ref{tab:v13-orthogonality}).}
  \item \textbf{v12b at $n \leq 1.5$\,\AA}: \emph{Answered---kicks
    transfer across all noise levels: $+$2.1/$+$4.9/$+$6.6\,pts at
    0.5/1.0/1.5\,\AA\ (Table~\ref{tab:v13-orthogonality}).}
  \item \textbf{Probe+late without adaptive}: \emph{Answered---263 vs.\
    267 ($-$4), adaptive adds marginal value
    (Table~\ref{tab:v13-ablation}).}
  \item \textbf{Late-escape timing}: Still open.  v13 used the same
    15k threshold.
  \item \textbf{Hard-core analysis}: \emph{Answered---most of the 36
    v12b hard-core samples converge at 50k+kicks.  Only 3 samples
    remain below 50\% convergence across 15 configs.}
\end{itemize}

%=============================================================================
\subsection{v13 Results (mar8 Run, March 2026)}
\label{sec:v13}
%=============================================================================

The v13 grid tested the central hypothesis remaining from v12b: that
50k steps and probe kicks are \textbf{orthogonal wins}---50k rescues
slow convergers while probe kicks rescue spectrally trapped samples.
These are different failure populations, so combining them should yield
additive gains.  Additionally, v13 extended the probe/late-escape
mechanisms to all noise levels (previously tested only at $n=2.0$\,\AA)
and included ablation studies at $n=2.0$.

\textbf{Design:} 15 configurations (3 main $\times$ 4 noise + 3
ablations at $n=2.0$), all RFO+PLS, trust floor 0.01\,\AA, single seed
(job~1149428), $N=287$.

\subsubsection{v13 Convergence Table}
\label{sec:v13-table}

\begin{table}[h]
\centering
\caption{v13 main grid: 3 configs $\times$ 4 noise levels ($N=287$, strict
  $n_\text{neg}=0$).  ``Full v12b'' = adaptive kick scaling + probe kicks
  + late escape.  All configs use RFO+PLS.  Three samples at $n=0.5$ and
  two at $n=1.0$ produce SCINE errors, consistent with prior runs.}
\label{tab:v13}
\begin{tabular}{lcccc}
\toprule
Configuration & 0.5\,\AA & 1.0\,\AA & 1.5\,\AA & 2.0\,\AA \\
\midrule
RFO+PLS 20k (baseline) & 272 (94.8\%) & 252 (87.8\%) & 224 (78.0\%) & 182 (63.4\%) \\
RFO+PLS 50k            & 275 (95.8\%) & 264 (92.0\%) & 258 (89.9\%) & 232 (80.8\%) \\
Full v12b 50k           & \textbf{281} (97.9\%) & \textbf{278} (96.9\%) & \textbf{277} (96.5\%) & \textbf{267} (93.0\%) \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{The Orthogonality Result}
\label{sec:v13-orthogonality}

The v13 main finding is that 50k steps and spectral probes are indeed
orthogonal interventions---their gains stack additively:

\begin{table}[h]
\centering
\caption{Decomposition of v13 gains at each noise level.
  ``$\Delta$~(50k)'' = 50k baseline minus 20k baseline.
  ``$\Delta$~(kicks)'' = full v12b 50k minus 50k baseline.
  Both effects are additive within this seed.}
\label{tab:v13-orthogonality}
\begin{tabular}{ccccc}
\toprule
Noise & $\Delta$~(50k) & $\Delta$~(kicks) & Total $\Delta$ & Final \\
\midrule
0.5\,\AA & $+$3 ($+$1.0\,pts) & $+$6 ($+$2.1\,pts) & $+$9 & 281 (97.9\%) \\
1.0\,\AA & $+$12 ($+$4.2\,pts) & $+$14 ($+$4.9\,pts) & $+$26 & 278 (96.9\%) \\
1.5\,\AA & $+$34 ($+$11.8\,pts) & $+$19 ($+$6.6\,pts) & $+$53 & 277 (96.5\%) \\
2.0\,\AA & $+$50 ($+$17.4\,pts) & $+$35 ($+$12.2\,pts) & $+$85 & 267 (93.0\%) \\
\bottomrule
\end{tabular}
\end{table}

The additivity confirms the theoretical prediction: 50k steps primarily
resolve oscillating failures (slow convergers that need more time), while
probe kicks resolve blind-mode failures (spectrally trapped samples
where gradient-Hessian methods cannot make progress regardless of time).
These two mechanisms target disjoint subpopulations.

At $n=2.0$\,\AA, the total gain from the 20k baseline is $+$85 samples
($+$29.6\,pts), decomposing cleanly into $+$50 from more steps and $+$35
from spectral probes.

\subsubsection{n=2.0 Ablation Analysis}
\label{sec:v13-ablation}

Three additional configs at $n=2.0$ dissect the kick mechanisms at 50k:

\begin{table}[h]
\centering
\caption{$n=2.0$\,\AA\ ablation at 50k steps ($N=287$).
  ``adapt'' = adaptive kick scaling, ``probe'' = blind-kick line probe,
  ``late'' = late-stage escape.  $\Delta$ is relative to the 50k bare
  baseline (232 converged).}
\label{tab:v13-ablation}
\begin{tabular}{lccl}
\toprule
Configuration & Converged & $\Delta$ vs.\ bare & Key insight \\
\midrule
50k bare (no kicks)          & 232 (80.8\%) & --- & --- \\
$+$adapt$+$probe (no late)   & 253 (88.2\%) & $+$21 & Probe dominates \\
$+$probe$+$late (no adapt)   & 263 (91.6\%) & $+$31 & Late still needed \\
$+$adapt$+$probe$+$late      & 267 (93.0\%) & $+$35 & Full suite \\
\midrule
20k $+$adapt$+$probe$+$late  & 213 (74.2\%) & --- & Step budget critical \\
\bottomrule
\end{tabular}
\end{table}

Three key conclusions from the ablation:

\begin{enumerate}[nosep]
  \item \textbf{Late escape is still needed at 50k}: Adding late escape
    to adaptive+probe gains $+$14 converged (253$\to$267).  The v12b
    hypothesis that ``50k might make late escape unnecessary'' is
    \textbf{falsified}.  Late escape rescues samples that remain
    spectrally trapped even after 15k--20k steps, which 50k alone cannot
    resolve because the blind-mode displacement rate
    (Eq.~\ref{eq:blind-mode-rate}) does not improve with more steps.
  \item \textbf{Adaptive scaling remains marginal}: $+$4 converged
    (263$\to$267), consistent with v12b's $+$1 at 20k.  However, the
    autopsy reveals a targeted effect: adaptive scaling rescues 8
    blind-mode failures (almost\_converged+ghost\_modes: 10$\to$2) at the
    cost of 5 additional oscillating failures (13$\to$18), for a net
    $+$4.  Adaptive scaling trades oscillating stability for blind-mode
    resolution.
  \item \textbf{Step budget is critical}: Full v12b kicks at 20k achieve
    only 213/287 (74.2\%) vs.\ 267/287 (93.0\%) at 50k---a gap of $+$54
    converged ($+$18.8\,pts).  The 50k budget is essential because it
    provides the time needed for both the oscillating population to
    converge naturally and the kick mechanisms to fire and take effect.
\end{enumerate}

\subsubsection{Failure Autopsy: Elimination of Failure Modes}
\label{sec:v13-autopsy}

\begin{table}[h]
\centering
\caption{Failure mode counts at $n=2.0$\,\AA\ across v13 configs.  ``Full''
  = adaptive+probe+late.  Compare rightmost row to v12b (20k):
  54/6/4/5 oscillating/AC/GM/SC.}
\label{tab:v13-autopsy}
\begin{tabular}{lccccc}
\toprule
Config & Osc & AC & GM & SC & Total fail \\
\midrule
20k baseline            & 76 & 17 & 7 & 4$+$1$^*$ & 105 \\
50k baseline            & 36 & 13 & 4 & 2 & 55 \\
$+$adapt$+$probe, 50k  & 26 & 3  & 5 & 0 & 34 \\
$+$probe$+$late, 50k   & 13 & 6  & 4 & 1 & 24 \\
Full v12b, 50k          & 18 & 2  & \textbf{0} & \textbf{0} & 20 \\
Full v12b, 20k          & 58 & 7  & 4 & 5 & 74 \\
\bottomrule
\multicolumn{6}{l}{\footnotesize $^*$1 drifting failure at 20k baseline.} \\
\end{tabular}
\end{table}

The full v12b config at 50k achieves a dramatic simplification of the
residual failure population at $n=2.0$\,\AA:

\begin{itemize}[nosep]
  \item \textbf{Ghost modes: eliminated} (7$\to$0).  The spectral probe
    directly targets the $n_\text{neg}$ criterion, and at 50k the probe
    has sufficient firing opportunities to resolve all ghost-mode cases.
  \item \textbf{Slow convergence: eliminated} (4$\to$0).  The extended
    budget resolves all marginally slow trajectories.
  \item \textbf{Almost converged: nearly eliminated} (17$\to$2).  These
    were the ``gradient-orthogonal but near threshold'' failures;
    the probe resolves most.
  \item \textbf{Oscillating: reduced 76$\to$18} (76\% reduction) but
    still dominant.  Oscillating failures now constitute 90\% (18/20) of
    remaining failures at $n=2.0$\,\AA.
\end{itemize}

The autopsy at other noise levels confirms near-complete resolution:

\begin{table}[h]
\centering
\caption{Failure modes for the full v12b 50k config across all noise levels.
  $N_\text{proc}$ accounts for SCINE errors (3 at 0.5, 2 at 1.0, 0 at
  1.5/2.0).}
\label{tab:v13-autopsy-all}
\begin{tabular}{ccccccc}
\toprule
Noise & $N_\text{proc}$ & Conv & Osc & AC & GM & Total fail \\
\midrule
0.5\,\AA & 284 & 281 (98.9\%) & 1 & 2 & 0 & 3 \\
1.0\,\AA & 285 & 278 (97.5\%) & 6 & 1 & 0 & 7 \\
1.5\,\AA & 287 & 277 (96.5\%) & 5 & 4 & 1 & 10 \\
2.0\,\AA & 287 & 267 (93.0\%) & 18 & 2 & 0 & 20 \\
\bottomrule
\end{tabular}
\end{table}

The performance cliff has been largely eliminated: the gap between
$n=0.5$ and $n=2.0$ is now 5.9\,pts (98.9\%$\to$93.0\%), compared to
51\,pts in v9 (80\%$\to$29\%), 47\,pts in v10c (89\%$\to$42\%), and
31\,pts in v11 (95\%$\to$64\%).

\subsubsection{Comparison Across All Versions}
\label{sec:v13-comparison}

\begin{table}[h]
\centering
\caption{Best convergence rate at $n=2.0$\,\AA\ across all versions, showing
  the cumulative effect of each intervention.  Same dataset ($N=287$), different
  seeds.}
\label{tab:v13-history}
\begin{tabular}{lcrl}
\toprule
Version & Best at 2.0\,\AA & $\Delta$ vs.\ prior & Key intervention \\
\midrule
v9 (GQT+TR, 10k) & 29\% & --- & Baseline \\
v10c (RFO+PLS, 10k) & 45\% & $+$16 & PLS \\
v11 (RFO+PLS, 20k) & 64\% & $+$19 & 20k steps \\
v12 (RFO+PLS+kicks, 20k) & 67\% & $+$3 & Fixed kicks \\
v12b (RFO+PLS+probe+late, 20k) & 76\% & $+$9 & Probe kicks \\
\textbf{v13 (full, 50k)} & \textbf{93\%} & \textbf{$+$17} & \textbf{50k $+$ kicks combined} \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Updated Best-Ever Table}
\label{sec:v13-best-ever}

\begin{table}[h]
\centering
\caption{Best strict convergence rate at each noise level across all runs
  and seeds ($N=287$).  Updated through v13.  Bold = new record vs.\ v12b.}
\label{tab:best-ever-v13}
\begin{tabular}{ccll}
\toprule
Noise & Best rate & Configuration & Run \\
\midrule
...
1.0\,\AA & 256/287 (89.2\%) & RFO+PLS (20k)        & v12 \\
1.5\,\AA & 230/287 (80.1\%) & RFO+PLS (20k)        & v12 \\
2.0\,\AA & 218/287 (76.0\%) & RFO+PLS+probe+late (20k) & v12b \\
\midrule
\multicolumn{4}{l}{\emph{50k steps}} \\
0.5\,\AA & \textbf{281/287 (97.9\%)} & RFO+PLS+full~v12b (50k) & \textbf{v13} \\
1.0\,\AA & \textbf{278/287 (96.9\%)} & RFO+PLS+full~v12b (50k) & \textbf{v13} \\
1.5\,\AA & \textbf{277/287 (96.5\%)} & RFO+PLS+full~v12b (50k) & \textbf{v13} \\
2.0\,\AA & \textbf{267/287 (93.0\%)} & RFO+PLS+full~v12b (50k) & \textbf{v13} \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{v13 Summary}
\label{sec:v13-summary}

\textbf{Main conclusions:}
\begin{enumerate}[nosep]
  \item \textbf{The orthogonality hypothesis is confirmed}: 50k steps and
    spectral probes target different failure populations (slow convergers
    vs.\ spectrally trapped) and their gains stack additively.  The combined
    effect at $n=2.0$ is $+$85 converged over the 20k baseline.
  \item \textbf{93\% at $n=2.0$\,\AA}: New best-ever by $+$17\,pts over
    v12b (76\%).  From 0\% (off-the-shelf) to 93\% with the same dataset
    and calculator.
  \item \textbf{Near-uniform convergence}: 97.9/96.9/96.5/93.0\% across
    noise levels.  The gap from 0.5 to 2.0\,\AA\ shrinks from 51\,pts
    (v9) to 5\,pts (v13).
  \item \textbf{Ghost modes and slow convergence eliminated}: Zero
    failures in either category at $n=2.0$ with the full suite.  Only
    oscillating remains (18/20 failures).
  \item \textbf{Late escape needed even at 50k}: Falsifies the hypothesis
    that more steps alone resolve the spectral trap.  The blind-mode
    displacement rate (Eq.~\ref{eq:blind-mode-rate}) is step-count
    independent, so only direct spectral perturbation can break the trap.
  \item \textbf{The intervention hierarchy is complete}: PLS $\gg$
    50k steps $\gg$ probe kicks $>$ late escape $\gg$ adaptive scaling
    $\approx$ fixed kicks $\approx$ GDIIS $\approx$ ARC.
\end{enumerate}

\textbf{Remaining failure population}: 20 failures at $n=2.0$\,\AA\
(18 oscillating, 2 almost\_converged).  Three samples remain hardest across
all 15 configs: 136 (6/15, $\lambda_\text{min}=-0.19$, strongly negative
eigenvalue resistant to all interventions), 150 (6/15), and 209 (7/15,
$n_\text{neg}=5$).  These represent molecules with persistent,
strongly negative eigenvalues that resist both time-based and spectral
interventions---likely requiring qualitatively different approaches
(e.g., internal coordinates, mode-following restarts, or alternative
starting geometries).
...
\subsection{v13 Grid (50k + v12b Kicks Combined, 15 configs)}

All configs use RFO+PLS.  Main grid: 3 configs (20k baseline, 50k
baseline, full v12b 50k) $\times$ 4 noise levels = 12 combos.
Three ablations at $n=2.0$: adapt+probe (no late), probe+late (no adapt),
full v12b at 20k.  Single run (job~1149428), $N=287$.

\textbf{Key findings} (full analysis in Section~\ref{sec:v13}):
\begin{itemize}[nosep]
  \item 50k + kicks are orthogonal: gains stack additively
    (Table~\ref{tab:v13-orthogonality}).
  \item \textbf{New best-ever at all noise}: 281/278/277/267 (97.9/96.9/96.5/93.0\%).
  \item Late escape still needed at 50k ($+$14 converged).
  \item Ghost modes and slow convergence eliminated at $n=2.0$.
  \item Only 20 failures remain at $n=2.0$ (18 oscillating, 2 almost\_converged).
\end{itemize}

\subsection{Recommended Configuration for Future Work}

Based on all evidence through v13, the recommended baseline is:
\begin{center}
\fbox{\textbf{RFO + PLS + adaptive + probe kicks + late escape, 50k steps}}
\end{center}
This is the v13 champion configuration, achieving 97.9/96.9/96.5/93.0\%
across all noise levels (Table~\ref{tab:v13}).  Schlegel trust radius
updates are neutral when PLS is active (Section~\ref{sec:v11-factors})
and can be omitted.  The trust radius floor must remain at 0.01\,\AA\
(lowering to 0.005\,\AA\ is catastrophic, Table~\ref{tab:trf-effect}).

For throughput-constrained applications, RFO+PLS+probe+late at 20k steps
achieves 76.0\% at $n=2.0$ (Table~\ref{tab:v12b}).  The 50k budget adds
$+$17\,pts at $n=2.0$ (Table~\ref{tab:v13-orthogonality}) with
diminishing returns at lower noise ($+$1\,pt at $n=0.5$).
Adaptive kick scaling adds only $+$4 converged (Section~\ref{sec:v13-ablation})
and can be omitted for simplicity.
...
\begin{enumerate}
  \item \textbf{Persistently hard samples}: \emph{Substantially resolved
    by v13.}  The v12b ``hard core'' of 36 samples (failing all 5 configs
    at $n=2.0$, 20k) is largely resolved at 50k: even the hardest samples
    (136, 150, 209) converge in 6--7/15 v13 configs.  Only sample~136
    ($\lambda_\text{min}=-0.19$, strongly negative) and sample~209
    ($n_\text{neg}=5$) remain genuinely resistant.  Spectral signature:
    persistent deep negative eigenvalues with low gradient overlap
    (Table~\ref{tab:hardest-10}).
  ...
  \item \textbf{Population~A (blind modes) resolved by v13}:
    \emph{Resolved.}  At $n=2.0$\,\AA\ with full v12b suite at 50k
    (Table~\ref{tab:v13-autopsy}): ghost\_modes = \textbf{0},
    slow\_convergence = \textbf{0}, almost\_converged = 2.  The
    spectral probe at 50k has sufficient firing opportunities to
    resolve virtually all blind-mode failures.  The sole remaining
    failure class is oscillating (18/20 failures).
  ...
  \item \textbf{Optimal step budget}: \emph{Resolved by v13.}
    50k + v12b kicks combined achieves 93.0\% at $n=2.0$\,\AA\
    (Table~\ref{tab:v13}), far exceeding the 80\% target.  The
    50k budget contributes $+$50 converged and kicks contribute
    $+$35 converged, confirming orthogonality
    (Table~\ref{tab:v13-orthogonality}).  The remaining 20 failures
    (90\% oscillating) may require 100k+ steps or qualitatively
...
%=============================================================================
\section{Discussion: Contributions and Generalizability}
\label{sec:discussion}
%=============================================================================

\subsection{The Problem Is Unsolved}
\label{sec:unsolved}

A critical piece of context, easily lost in the version-by-version
narrative above, is that \textbf{no existing optimizer converges on this
problem class out of the box}.  Standard trust-region Newton--Raphson
implementations (e.g., ASE's \texttt{BFGSLineSearch}, SciPy's
\texttt{trust-ncg}) achieve 0\% strict convergence on Transition1x
samples displaced by $\geq 1.0$\,\AA\ under DFTB0.  Even with
aggressive hand-tuning, conventional trust-region codes reach
$\sim$20--30\%.  Sella~\citep{sella}---the current state of the art,
using RS-RFO with geodesic back-transformation in internal
coordinates---achieves 10--20\% with careful parameter selection.

Against this baseline, our results look very different:

\begin{center}
\begin{tabular}{lccccc}
\toprule
Method & 0.5\,\AA & 1.0\,\AA & 1.5\,\AA & 2.0\,\AA & Notes \\
\midrule
Off-the-shelf TR       & $\sim$30\% & $\sim$20\% & $<$10\% & 0\% & Out of box \\
Sella (tuned)          & $\sim$20\% & $\sim$15\% & $\sim$10\% & $<$10\% & Best published code \\
\midrule
Our v9 (GQT+TR)       & 80\%  & 60\%  & 46\%  & 29\%  & Shifted Newton, 10k \\
Our v10c (RFO+PLS)    & 89\%  & 78\%  & 63\%  & 45\%  & $+$PLS, 10k \\
Our v11 (RFO+PLS,20k) & 95\%  & 85\%  & 77\%  & 64\%  & $+$20k steps \\
Our v12b (full,20k)    & ---   & ---   & ---   & 76\%  & $+$probe$+$late \\
\textbf{Our v13 (full,50k)} & \textbf{98\%} & \textbf{97\%} & \textbf{97\%} & \textbf{93\%} & \textbf{50k + probe + late} \\
\bottomrule
\end{tabular}
\end{center}

The gap is not incremental---it is a factor of $3$--$10\times$ at high
noise, and v13 widens it further: 93\% vs.\ $<$10\% at $n=2.0$\,\AA.
This establishes that the problem is \emph{genuinely unsolved}
by existing tools, and the interventions documented here are not
marginal improvements on a well-studied pipeline but the difference
between broken and functional.

\subsection{Three Contributions}
\label{sec:contributions}

From the experimental record, three results stand out as contributions
to the broader optimization literature, not merely to this codebase.

\subsubsection{The Convergence Criterion Gap ($\varepsilon^{1/2}$ Insight)}
\label{sec:eps-half-insight}

\begin{center}
\fbox{\parbox{0.85\textwidth}{%
  If the optimizer drives $\norm{\vect{g}} \leq \varepsilon$, the theory
  guarantees $\lambda_\text{min} \geq -\sqrt{\varepsilon}$.  Demanding
  $n_\text{neg}=0$ asks for more than the mathematics promises.  The
  entire noise-dependent performance cliff---from 95\% at 0.5\,\AA\ to
  44\% at 2.0\,\AA---is explained by samples sitting in
  $[-\sqrt{\varepsilon},\, 0)$ that are theoretically acceptable but
  rejected by the strict criterion.%
}}
\end{center}

With $\varepsilon = 10^{-4}$\,eV/\AA, the predicted bound is
$\sqrt{10^{-4}} = 0.01$---exactly our empirically optimal relaxed
threshold.  At this threshold, convergence is $\sim$90\% at
\emph{all} noise levels, and the performance cliff vanishes.

These two parameters ($\varepsilon$ and $\tau_\text{relax}$) were set
independently: the force threshold by convention, the eigenvalue
threshold by grid search over the cascade.  That they satisfy
$\tau = \sqrt{\varepsilon}$ is an independent empirical rediscovery of
the Boumal et al.~\citep{boumal-arc-manifolds} guarantee.  The
implication is practical: \textbf{any code that converges forces to
$\varepsilon$ should accept eigenvalues down to $-\sqrt{\varepsilon}$},
not demand strict positive definiteness.  The standard $n_\text{neg}=0$
criterion used throughout computational chemistry is more strict than
what second-order optimization theory provides.

\subsubsection{The Gradient-Orthogonality Trap (Blind Modes)}
\label{sec:blind-mode-insight}

The standard narrative in nonconvex optimization is: ``second-order
methods escape saddle points because they use curvature information.''
Our data shows this is incomplete.

\begin{center}
\fbox{\parbox{0.85\textwidth}{%
  When the gradient $\vect{g}$ is orthogonal to a negative eigenvector
  $\vect{v}_i$ (i.e., $\vect{g} \cdot \vect{v}_i \approx 0$), the
  Newton step component along $\vect{v}_i$ is
  $s_i = -c_i/(\lambda_i + \sigma) \approx 0$ regardless of how
  negative $\lambda_i$ is.  The optimizer knows exactly what is wrong
  (from the Hessian) and exactly where (from the eigenvector), but
  cannot act on it because its only steering signal---the
  gradient---has no projection onto the problematic direction.%
}}
\end{center}

This is the \textbf{blind-mode trap}: having the exact Hessian does
not help when the gradient is uninformative.  In our data, 30--50\% of
non-oscillating failures at $n=2.0$\,\AA\ exhibit this signature
(gradient overlap $<0.1$ with the most negative eigenvector,
Table~\ref{tab:hardest-10}).  The trap is not a numerical artifact or a
dataset quirk---it is a geometric property of gradient flow on any
surface where the gradient vector lies in a subspace orthogonal to the
negative eigenspace of the Hessian.

The per-step displacement in the blind direction can be bounded
quantitatively.  If $\delta_i = |\vect{g}\cdot\vect{v}_i|$ is the
gradient overlap with negative eigenvector $\vect{v}_i$, the Newton
step component along $\vect{v}_i$ is:
\begin{equation}
  |s_i| = \frac{\delta_i}{|\lambda_i| + \sigma}.
  \label{eq:blind-mode-rate}
\end{equation}
In the blind regime, $\delta_i \leq 0.05 \times \norm{\vect{g}}
\leq 5\times10^{-6}$\,eV/\AA\ and $|\lambda_i| + \sigma \sim 10^{-3}$,
so $|s_i| \leq 5\times10^{-3}$\,\AA\ before trust-radius clipping.
At the trust floor ($\Delta = 0.01$\,\AA), the actual per-atom
displacement in the blind direction is further reduced to
$\sim\!10^{-5}$\,\AA/step.  To effect an eigenvalue sign change
requires a structural displacement of $O(10^{-2})$--$O(10^{-1})$\,\AA\
along $\vect{v}_i$, implying $10^{3}$--$10^{4}$ steps \emph{at minimum}
---consistent with the trajectory data showing permanently hard samples
make negligible spectral progress even after 20k steps
(Section~\ref{sec:trajectory-analysis}).

Trust-region methods cannot escape this trap because the trust radius
constrains the step to the model's prediction, which is near-zero in
the blind direction.  The bound~\eqref{eq:blind-mode-rate} makes
``more steps'' a losing strategy: the convergence time scales as
$O(1/\delta_i)$, which diverges as the overlap vanishes.  This
explains why more steps alone cannot resolve blind-mode failures---only
the spectral probe (Section~\ref{sec:spectral-probe-insight}), which
bypasses the gradient entirely, can break the trap.  v13 confirms:
ghost modes and slow convergence are eliminated by the probe at 50k,
while late escape is still needed even at 50k
(Table~\ref{tab:v13-ablation}).

\subsubsection{The Spectral Probe}
\label{sec:spectral-probe-insight}

The fix for the blind-mode trap is to bypass the gradient entirely:

\begin{center}
\fbox{\parbox{0.85\textwidth}{%
  Probe a distance ladder along the negative eigenvector $\vect{v}_i$
  in both $\pm$ directions.  Accept the first displacement where
  $n_\text{neg}$ decreases.  This is a \textbf{spectral line search}:
  a 1D search whose merit function is the Hessian spectrum, not the
  energy.%
}}
\end{center}

The spectral probe is the algorithmic contribution of this work.  It
can be understood in the context of the saddle-escape literature: Jin
et al.~\citep{jin-perturbed-gd} proved that adding isotropic random
noise to gradient descent suffices to escape strict saddle points in
polynomial time.  Their mechanism relies on the noise having a nonzero
projection onto the escape direction with constant probability---a
brute-force fix that works in expectation but wastes perturbations in
non-escape directions, producing $\sim$1:1 rescue-to-loss ratios (as
our v12 fixed-kick experiments confirmed).

The spectral probe is the \textbf{second-order analog of perturbed GD}:
instead of random perturbation in a random direction, it perturbs along
the \emph{known} negative eigenvector.  Instead of an energy-decrease
acceptance criterion (which is uninformative in the blind regime---the
energy is already nearly converged), it uses an
$n_\text{neg}$-decrease criterion that directly targets the spectral
deficiency.  This directed approach yields a 2.4:1 rescue-to-loss ratio
(probe kicks) and 3.2:1 (late escape), compared to $\sim$1:1 for
undirected perturbations.  The improvement is possible precisely because
we have second-order information (the full Hessian): we know \emph{which}
direction to search, we just lack the gradient signal to do so within the
standard Newton framework.

The probe is the natural counterpart to trust-region exploitation:
where the trust region follows the model (exploit), the probe searches
along the eigenvector (explore).  It is activated only when the
gradient-based machinery is provably ineffective (blind-mode
detection, overlap $<0.1$ with $|\lambda_i|<\sqrt{\varepsilon}$),
making it a targeted intervention rather than random noise injection.

\subsection{Generalizability}
\label{sec:generalizability}

\subsubsection{What Transfers: Mathematical Principles}

Every major finding maps to a general mathematical mechanism, not a
dataset-specific coincidence:

\begin{longtable}{p{0.30\textwidth}p{0.65\textwidth}}
\toprule
\textbf{Finding} & \textbf{Why It Generalizes} \\
\midrule
\endhead

PLS is the biggest single win
& Cubic interpolation stabilizes eigenvalue-crossing regimes---a
  property of \emph{any} PES with mixed curvature.  PLS is already
  standard in production codes; our contribution is quantifying its
  dominance ($+$7 to $+$34\,pts). \\

Trust floor = $\sqrt{\varepsilon}$
& Boumal et al.'s theorem applies to any second-order method on any
  smooth function.  The specific value 0.01\,\AA\ is
  $\varepsilon$-dependent, but the scaling law transfers. \\

ARC fails on noisy PES
& $\rho$-ratcheting occurs whenever the quadratic model's prediction
  error has a noise floor (semi-empirical, ML potentials, DFT with
  loose SCF).  Not DFTB0-specific. \\

GDIIS has zero effect
& $k$-vector history cannot capture oscillation in $d$-DOF systems
  when $k \ll d$.  Gets \emph{worse} for larger molecules. \\

Blind-mode trap
& Gradient--eigenvector orthogonality is a geometric property of
  any surface.  Independent of calculator, coordinates, or molecule
  size. \\

Spectral probe
& $n_\text{neg}$-decrease acceptance is meaningful on any PES where
  eigenvalues can be computed.  Distances are parameterized as
  multiples of $\sqrt{\varepsilon}$, so the algorithm adapts to the
  force threshold. \\

\bottomrule
\end{longtable}

The hierarchy of interventions---PLS $\gg$ 50k steps $\gg$ spectral
probe $>$ late escape $\gg$ adaptive scaling $\approx$ fixed kicks
$\approx$ GDIIS $\approx$ ARC---is confirmed by v13's additive
decomposition (Table~\ref{tab:v13-orthogonality}) and reflects
fundamental properties of optimization on mixed-curvature surfaces,
not Transition1x-specific tuning.

\subsubsection{What Does Not Transfer: Parameters and Regime}

Three aspects of this work are problem-specific:

\begin{enumerate}[nosep]
  \item \textbf{Parameter values}: The trust floor (0.01\,\AA), force
    threshold ($10^{-4}$\,eV/\AA), kick magnitudes, patience/cooldown
    constants, and late-escape activation step (15k) are calibrated to
    small organic molecules on DFTB0.  Different PES scales require
    retuning, though the $\sqrt{\varepsilon}$ scaling law provides a
    principled starting point.
  \item \textbf{Molecular size regime}: With $\leq$23 atoms ($\leq$69
    DOF), full analytical Hessians are feasible at every step.  For
    larger systems requiring L-BFGS or model Hessians, the spectral
    decomposition, $n_\text{neg}$ counting, and mode tracking that
    underpin the probe and late-escape mechanisms are not directly
    applicable.
  \item \textbf{Starting-point distribution}: Displacing from known
    transition states produces a specific distribution of initial
    Hessian spectra (median $n_\text{neg} \approx 12$,
    $\kappa \sim 10^{4}$--$10^{6}$).  Starting from random geometries,
    ML-predicted structures, or force-field pre-optimized structures
    would give different initial conditions and potentially different
    failure mode distributions.
\end{enumerate}

\subsubsection{Evidence Against Overfitting}

The strongest evidence that these findings are not overfit to
Transition1x:

\begin{enumerate}[nosep]
  \item \textbf{Independent parameter discovery}: The force threshold
    $\varepsilon = 10^{-4}$ and relaxed eigenvalue threshold
    $\tau = 0.01$ were set independently by different criteria (force
    convergence convention vs.\ cascade grid search).  Their
    relationship $\tau = \sqrt{\varepsilon}$ was discovered post hoc
    from the literature.  Overfitting does not produce theoretically
    predicted parameter relationships.
  \item \textbf{Cross-seed stability}: Failure mode distributions
    reproduce across random seeds within $\pm$1--4\,pts
    (Table~\ref{tab:reproducibility}, Table~\ref{tab:autopsy-cross-seed}).
    Oscillating counts at matched noise levels are identical across
    seeds (e.g., 76 vs.\ 76 at $n=1.5$).
  \item \textbf{Monotonic noise scaling}: Every intervention's effect
    scales smoothly with displacement magnitude, consistent with a
    physical mechanism (increasing Hessian ill-conditioning,
    Table~\ref{tab:condition}) rather than dataset-specific structure.
  \item \textbf{Negative results explained by theory}: ARC failure
    ($\rho$-ratcheting), GDIIS failure (dimensional mismatch), line
    search failure (poor direction quality), trust-floor sensitivity
    ($\varepsilon^{1/2}$ bound)---all predicted by existing theory
    before we observed them.  If we were overfitting, we would find
    unexplainable patterns, not theory-confirming ones.
  \item \textbf{Other optimizers fail on the same task}: The comparison
    with off-the-shelf codes (0--30\%) and Sella (10--20\%) establishes
    that the difficulty is intrinsic to the problem, not an artifact of
    a weak baseline.  Our interventions address the intrinsic difficulty,
    not a codebase-specific bug.
  \item \textbf{Additive decomposition of mechanisms}: v13's orthogonality
    result (Table~\ref{tab:v13-orthogonality}) shows that 50k steps and
    spectral probes target disjoint failure populations (oscillating vs.\
    blind modes) and their gains stack additively.  Overfitting to a
    dataset produces correlated improvements; mechanistically distinct
    interventions with additive effects reflect genuine population
    structure.
\end{enumerate}

\subsubsection{Implications for Machine Learning Potentials}

The ML-for-chemistry community increasingly uses learned potentials
(NequIP, MACE, PaiNN, etc.)\ trained on datasets like Transition1x and
then minimizes on the learned PES to produce equilibrium structures.
Our findings have direct implications:

\begin{itemize}[nosep]
  \item \textbf{ARC will fail harder}: ML potentials have larger Hessian
    noise floors than DFTB0 ($\sim$8$\times10^{-3}$).  The
    $\rho$-ratcheting mechanism that killed ARC on DFTB0 will be
    \emph{more} severe on noisier potentials.
  \item \textbf{PLS is even more important}: Eigenvalue crossings and
    poor quadratic models are more frequent on rougher PES.  PLS's
    zero-cost step-length correction becomes essential, not optional.
  \item \textbf{The $\sqrt{\varepsilon}$ criterion is necessary}: If
    practitioners demand $n_\text{neg}=0$ on an ML potential with a
    force RMSE of $\sim$0.05\,eV/\AA, the Hessian noise alone will
    produce persistent ghost negatives.  The relaxed criterion
    ($\lambda_\text{min} \geq -\sqrt{\varepsilon}$) avoids conflating
    numerical noise with genuine saddle character.
  \item \textbf{Full Hessians may not be available}: The spectral probe
    and blind-mode detection require eigendecomposition of the full
    Hessian, which is $O(N^3)$ and often unavailable for ML potentials.
    An open question is whether approximate spectral probes (e.g., using
    Lanczos iteration for the lowest eigenvalue only) retain the directed
    advantage over random perturbation.
\end{itemize}

The most direct generalizability test would be to swap DFTB0 for
GFN2-xTB (available through the same SCINE interface, different PES
characteristics) or an ML potential and verify that the intervention
hierarchy holds.  This is noted as future work.
...
\begin{itemize}[nosep]
  ...
  \item \texttt{scripts/slurm\_templates/} --- SLURM job scripts (v3--v13)
\end{itemize}
...
\bibitem[Jin et al.(2017)]{jin-perturbed-gd}
  C.~Jin, R.~Ge, P.~Netrapalli, S.~M.~Kakade, M.~I.~Jordan,
  ``How to escape saddle points efficiently,''
  \emph{Proc.\ 34th Int.\ Conf.\ Machine Learning (ICML)}, pp.~1724--1732, 2017.
